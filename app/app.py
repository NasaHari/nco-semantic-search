...
with col_text:
    ...
    if st.button("Search") or (st.session_state.query and st.session_state.search_triggered):
        st.session_state.search_triggered = True
...
with col_mic:
    ...
    if audio:
        ...
        if spoken_text:
            ...
            do_search()
            st.rerun()
INDIC_LANGS = {"hi", "ta", "te", "kn", "ml", "bn", "mr"}  # Add more if needed
    ...
    with st.spinner("Searching..."):
        if use_ensemble:
           # Use the new ensemble_search function
            results = ensemble_search(st.session_state.searcher, st.session_state.query, top_k=10)
            subheader_text = "Top matches (Ensemble)"
        else:
            detected_lang = detect(st.session_state.query)
            print(detected_lang)
            if detected_lang in INDIC_LANGS:
                results = st.session_state.searcher[MODEL_ALIASES['lbs']].search(st.session_state.query, top_k=10)
            else:
                results = st.session_state.searcher[MODEL_ALIASES['min']].search(st.session_state.query, top_k=10)
